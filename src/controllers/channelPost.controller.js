...
export const createChannelPost = asyncHandler(async (req, res) => {
    const validationResult = await createOrUpdateChannelPostValidatorSchema.safeParseAsync(req.body);
...
export const getUserChannelPosts = asyncHandler(async (req, res) => {
    ...
    if (!existingUser)
        throw new API_Error(404, `User with id ${userId} not found. Invalid user ID`);
...
export const updateChannelPost = asyncHandler(async (req, res) => {
    ...
    const existingPost = await ChannelPosts.findById(postId).select("-createdAt -owner");
    if (!existingPost)
        throw new API_Error(404, `Post does not exist!`);
